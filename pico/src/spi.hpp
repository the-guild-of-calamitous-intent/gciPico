...
public:
  ...
  bool init(const uint8_t port_num, uint32_t baud, const pin_t miso,
            const pin_t mosi, const pin_t sck, const pin_t cs) {
  ...
  inline // value? Do we need to change this?
      uint32_t
      set_baudrate(const uint32_t baud) {
    return spi_set_baudrate(port, baud);
  }
